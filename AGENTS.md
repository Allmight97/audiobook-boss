...
- For behavior/runtime, IPC, dependency/toolchain, build/test semantics, or
  release-surface changes, run `mise run proof` before sharing
  changes for review. See [`docs/greenproof.md`](docs/greenproof.md) for task map.
...
- Verification is explicit by risk:
  ...
  - boundary/backend/runtime behavior: `mise run proof` plus any
...
- Verification matches scope:
  ...
  - app behavior, contracts, dependency resolution, build/test semantics,
    release artifacts, or broad config semantics: `mise run proof`
