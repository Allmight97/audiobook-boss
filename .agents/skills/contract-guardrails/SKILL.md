...
```
3. Run `bun scripts/proof/runner.ts review` when the change affects Rust command/event
...
# or
CHECK_BINDINGS_STRICT=1 bun scripts/proof/runner.ts review
```
...
- Verification follows root `AGENTS.md` risk-based scope: focused boundary
  checks for local adapter changes; `bun scripts/proof/runner.ts review` for contract,
