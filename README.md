...
- For substantial planning and alignment, use `.agents/skills/decision-alignment`; default durable capture is GitHub issues (`docs/agents/issue-tracker.md`). Use `docs/specs/` only when explicitly requested. Session handoffs belong in OS temp via global `handoff`, not the repo. External presentation artifacts belong under `/Users/jstar/Documents/Codex/artifacts/audiobook-boss`.
