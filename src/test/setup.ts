...
vi.mock('@tauri-apps/api/core', () => ({
	invoke: vi.fn().mockImplementation((cmd: string, _args?: unknown) => {
		switch (cmd) {
			...
			case 'list_remote_source_providers':
				return Promise.resolve([
					{
						providerId: 'audible',
						label: 'Audible',
			...
			case 'get_remote_source_account_state':
				return Promise.resolve({
					...
					status: 'needsAuth',
					account: null,
					message: 'Connect Audible to load your library.',
				});
			case 'start_remote_source_auth':
				return Promise.resolve({
					providerId: 'audible',
					authorizationUrl: 'https://www.audible.com/mock-auth',
					handoffPathHint: '$TMPDIR/abb-audible-auth-response-url.txt',
					message: 'Open the authorization URL externally.',
				});
			...
			case 'refresh_remote_source_library':
				return Promise.resolve({
					providerId: 'audible',
					titles: [
						{
