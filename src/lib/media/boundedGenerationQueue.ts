...
/**
 * A small path/key queue for previews whose visible work changes as a list is
 * replaced. It bounds active work without pretending to abort already-started
 * requests; callers decide their own state transitions and commits.
 */
export function createBoundedGenerationQueue(concurrency: number): BoundedGenerationQueue {
	...
	function schedule<T>(
		...
		for (const key of uniqueKeys) {
			if (options.prepare(key, scheduleGeneration)) {
				queuedKeys.push(key);
			}
		}
		pump();
	...
	function pump(): void {
		...
		while (activeCount < concurrency && queuedKeys.length > 0) {
			...
			if (!key || !isCurrent(key, generation)) {
				continue;
			}
			activeCount += 1;
			let completed = false;
