...
/// Call `sync` whenever the owning `isOpen` state changes. Escape always
/// routes through the caller-supplied `onEscape` callback — the same
/// close/cancel path a visible Close/Cancel button uses — never a force-hide
/// here.
export class ModalController {
	...
	sync(open: boolean, elements: ModalElements, options: ModalSyncOptions): void {
		...
		this.#restoreInvoker = options.restoreInvoker !== false;
		if (open && !this.#isOpen) {
			this.#isOpen = true;
	...
	#handleCloseTransition(): void {
		...
		this.#invoker = null;
		if (this.#restoreInvoker && invoker && document.contains(invoker)) {
