...
const FILE_LIST_COVER_THUMBNAIL_CONCURRENCY = 2;
const MAX_FILE_LIST_COVER_THUMBNAIL_CACHE_ENTRIES = 64;
...
export function createFileListCoverThumbnails(loadThumbnail: FileListCoverThumbnailLoader) {
	...
	function read(path: string | null | undefined): FileListCoverThumbnailState {
		revision();
	...
	function schedule(paths: ReadonlyArray<string>): void {
		if (paths.length === 0) {
			clear();
			return;
		}
		thumbnailQueue.schedule(paths, {
			...
			start: (path, generation, complete) => startThumbnailFetch(path, generation, complete),
		});
	}
	...
	return { read, schedule, dispose: clear };
}
