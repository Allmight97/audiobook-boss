...
function isSubseriesPrefixed(value: string): boolean {
  ...
  return ["part", "book", "vol", "vol.", "volume"].some((prefix) =>
    trimmed.startsWith(prefix)
  );
}

function normalizeSubseriesLabel(
  subseries: string,
  subseriesPart: string | undefined
): string {
...
export function calculateOutputPath(metadata: AudiobookMetadata): string {
  ...
  if (jobType === "batch") {
    ...
    if (naming.absCompatible) {
      const subseriesLabel = normalizeSubseriesLabel(
        placeholderSubseries,
  ...
  if (naming.absCompatible) {
    ...
    if (subseries) {
      ...
      const subseriesPart =
        subseriesPartValue.length > 0 ? subseriesPartValue : undefined;
      ...
      const seriesPart =
        seriesPartValue.length > 0 ? seriesPartValue : undefined;
