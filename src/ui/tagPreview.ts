 ...
 */
const TAG_FIELD_MAPPINGS: Record<string, () => string> = {
...
/**
 * Updates a single tag preview field
 */
function updateTagField(field: string): void {
    const element = document.querySelector(`[data-field="${field}"]`);
    if (!element) return;

    const getter = TAG_FIELD_MAPPINGS[field];
    if (!getter) return;
...
export function updateTagPreview(): void {
    for (const field of Object.keys(TAG_FIELD_MAPPINGS)) {
        updateTagField(field);
    }
}
