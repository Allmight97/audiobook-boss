...
mod tests {
    ...
    fn mono_f32_frame(samples: &[f32]) -> ff::frame::Audio {
        ...
        let dst: &mut [f32] = unsafe {
            std::slice::from_raw_parts_mut(plane.as_mut_ptr() as *mut f32, samples.len())
        };
    ...
    fn f32_planar_preserves_order_across_multiple_consumption_cycles() {
        ...
        for chunk in expected.chunks(3) {
            ...
            for full in acc.push_frame(&frame) {
                ...
                let out: &[f32] = unsafe {
                    std::slice::from_raw_parts(plane.as_ptr() as *const f32, full.samples())
                };
        ...
        let tail_samples: &[f32] = unsafe {
            std::slice::from_raw_parts(tail_plane.as_ptr() as *const f32, tail.samples())
        };
