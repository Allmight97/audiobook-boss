...
function runSyntheticThroughput() {
	...
	const bytesProcessed = samples.byteLength * SYNTHETIC_WORKLOAD.passes;
	const mibPerSecond = elapsedMs > 0 ? bytesProcessed / (1024 * 1024) / (elapsedMs / 1000) : 0;
...
function runEncoderScenario({ fixture, clipSeconds, processedSeconds, scenario }) {
	...
	try {
		...
		const realtimeFactor = elapsedSeconds > 0 ? processedSeconds / elapsedSeconds : 0;
...
export const benchmark = {
	...
	description: 'Audio throughput benchmark using synthetic DSP and real encoder-path transcodes.',
