...
- UI islands are present, but core runtime paths (`fileList/**`, `statusPanel/**`, `encoderPanel/**`, `outputPanel/**`, `coverArt.ts`) still use imperative DOM orchestration.
- Metadata intent semantics are retained in boundary compile flows (`set|clear|noop`), but full runtime convergence is still blocked by remaining imperative file-list and status/processing lanes.
