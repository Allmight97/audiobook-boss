...
- Status-panel progress/status/concurrency/cancel-pending now mirror into reactive view state and are rendered from `StatusPanelIsland.svelte`, reducing direct DOM ownership pressure (`src/ui/statusPanel/viewState.svelte.ts`, `src/ui/statusPanel/StatusPanelIsland.svelte`, `src/ui/statusPanel/dom.ts`).
- Status-panel job-list runtime rendering no longer uses manual DOM diffing/row lifecycle bookkeeping; `renderJobList` is now reactive state-only (`src/ui/statusPanel/dom.ts`) and incremental behavior tests were moved to view-state assertions (`src/ui/statusPanel/__tests__/renderIncrements.test.ts`).
