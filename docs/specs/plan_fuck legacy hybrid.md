...
- UI islands are present, but core runtime paths (`statusPanel/**`, `encoderPanel/**`, `outputPanel/**`, `coverArt.ts`) still use imperative DOM orchestration; file-list runtime rendering is now island/state-driven with residual legacy helpers still present.
- Metadata intent semantics are retained in boundary compile flows (`set|clear|noop`), but full runtime convergence is still blocked by status/processing and wave-1 legacy lanes.
