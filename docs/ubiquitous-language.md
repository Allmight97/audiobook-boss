...
| **Media Contract** | Backend-neutral media facts, progress, error, and provenance vocabulary used before a concrete adapter executes. It must not expose FFmpeg, AAXClean, provider payload, or `mp4ameta` internals. | ffmpeg wrapper, provider response shape, metadata tool model |
