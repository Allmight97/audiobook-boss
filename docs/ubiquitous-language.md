...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Proof Route** | A named GreenProof task under `mise run …` that chooses the cheapest meaningful proof for the touched owner surface. | random command, local incantation |
| **Review Gate** | The main non-doc review proof route run via `mise run proof` to validate code, contracts, and repo health. | test pass, quick smoke check |
