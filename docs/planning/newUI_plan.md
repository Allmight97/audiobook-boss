...
**Backend persistence**: Extend `AudiobookMetadata` (Rust + TS) and the Lofty reader/writer so MVNM, MVIN, TSOA, Album Sort, and Series fields round-trip instead of being derived only for display. Keep the UI mapping in sync with those structs.
...
- Always resize to fit within 800×800 (preserve aspect ratio)
- Flatten transparency to white background before conversion
- Re-encode to JPEG at 85% quality regardless of source format
- Return optimized bytes for both user-selected and auto-loaded paths
- Call from `load_cover_art_file` **and** `read_audio_metadata` so every ingress path is standardized
...
#### 3.3 Add Tag Preview Grid Logic + Metadata Binding
...
- Persist the same calculated fields into `AudiobookMetadata` before invoking the backend (use new fields instead of overloading `album`)
- **Files**: `src/ui/outputPanel.ts`, `src/ui/statusPanel/logic.ts`, `src/ui/tagPreview.ts` (new helper)

#### 3.4 Update Advanced Settings Panel + Encoder Provider

- Wire encoder type/profile dropdowns to `EncoderSettingsV2` state (use existing `state.ts`)
- Keep VBR checkbox disabled (per mock) but honor stored setting when toggled later
- Remove keyboard shortcut handler (and hint text from HTML)
- Register `window.EncoderSettingsProvider = () => currentEncoderSettings` so `StatusPanel` payloads stop falling back to defaults
- **Files**: `src/ui/encoderPanel/*` (index.ts, state.ts, dom.ts, logic.ts), `src/ui/statusPanel/logic.ts` (ensure provider is read)
...
| `src-tauri/src/metadata/mod.rs`           | Modify      | Add series/title-sort fields to `AudiobookMetadata`                        |
| `src-tauri/src/metadata/writer.rs`        | Modify      | Persist MVNM/MVIN/TSOA/ALBUMSORT via Lofty                                 |
| `src-tauri/src/audio/settings_encoder.rs` | Modify      | Add bitrates 104, 120                                                      |
| `src/types/audio.ts`                      | Modify      | Update bitrate constant                                                    |
| `src/types/metadata.ts`                   | Modify      | Mirror new metadata fields                                                 |
...
| `src/ui/outputPanel.ts`                   | Modify      | Bitrate/sample rate dropdowns, `estimated-size` ID, tag preview helper     |
| `src/ui/statusPanel/logic.ts`             | Modify      | Preview duration state, `meta-series-part` ID, persist new metadata fields |
| `src/ui/statusPanel/dom.ts`               | Modify      | Split button DOM handling                                                  |
| `src/ui/fileList/actions.ts`              | Modify      | Update `meta-series-part` ID reference, populate new metadata fields       |
| `src/ui/encoderPanel/*`                   | Modify      | Wire advanced settings panel + expose EncoderSettingsProvider              |
| `src/ui/tagPreview.ts`                    | Create      | Centralized tag preview + metadata marshalling helper                      |
...
| Cover art paths        | Optimize BOTH user-selected AND auto-loaded from source files (single helper) |
...
- Create `optimize_cover_art(bytes: Vec<u8>) -> Result<Vec<u8>>`:
  - Always resize to fit within 800×800, flatten transparency, convert to JPEG 85%
  - Return optimized bytes for every cover art ingest path
- Call from `load_cover_art_file` command and `read_audio_metadata`
...
- **outputPanel.ts**: Update bitrate dropdown (48-128 step 8), sample rate options, change `output-estimated-size` → `estimated-size`, integrate tag preview helper
- **statusPanel/logic.ts**: Add `previewDuration` state, wire split button, change `meta-series-pos` → `meta-series-part`, persist new metadata fields
- **fileList/actions.ts**: Update `meta-series-pos` → `meta-series-part` reference, populate new metadata fields when loading existing files
- **tagPreview.ts**: New helper shared by output panel + metadata serialization to keep TSOA/MVNM logic DRY
- **encoderPanel/\***: Wire advanced settings (encoder type, profile, VBR disabled) and register `window.EncoderSettingsProvider`
