...
| Encoder registry and bitrate fields | `src-tauri/src/audio/settings_encoder.rs`; `src-tauri/src/audio/processor/encoder/common.rs`; encoder option modules | Query FFmpeg encoders by C name, set bitrate fields, and read opened native encoder AVOptions. | Encoder availability detection, AAC mode behavior, bitrate behavior, encode startup. | C strings are validated; borrowed encoder pointers stay live during the call; required NMR option readback failures or mismatches reject startup through the audio error boundary. |
